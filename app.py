...
def ajustar_volumen_m3(kilo_mayor, q_os, umbral_m3_por_os=1.0, m3_estandar_por_os=0.33):
    """Motor de ajuste de volumen para controlar cubicaciones erráticas.

    1) Convierte kilo mayor a m3.
    2) Calcula m3 por OS: volumen_m3 / q_os.
    3) Si m3_por_os supera el umbral, reemplaza el volumen por
       q_os * m3_estandar_por_os.
    ...
    Regla actual:
      si volumen_m3 / q_os > 1.0  => volumen_m3 = q_os * 0.33
    """
    ...
    volumen_final = volumen_original.mask(volumen_ajustado, os * m3_estandar_por_os).clip(lower=0)
...
def cargar_dicruta(file_or_path):
    ...
    return out.dropna(subset=["ruta"])[["ruta", "capacidad", "tipo_flota"]].drop_duplicates("ruta")
...
def preparar_data(peu_raw, accesos_raw):
    ...
    cols = dict(col_reserva=col_reserva, col_q_os=col_q_os, col_km=col_km, col_piezas=col_piezas, lat_col=lat_col, lon_col=lon_col)
...
def preparar_puntos(df, cols, max_capacidad=None):
    ...
    # Volumen/capacidad: el diccionario de rutas está en m3.
    # La demanda se convierte desde kilo mayor a m3 y luego se aplica
    # el motor de ajuste para cubicaciones erráticas.
    # Regla: si volumen_m3 / q_os > 1, se asume 0.33 m3 por OS.
    if col_km:
        ajuste_vol = ajustar_volumen_m3(d[col_km], d["_os"], umbral_m3_por_os=1.0, m3_estandar_por_os=0.33)
...
def construir_flotas_baseline(baseline, dicruta):
    ...
    rows = []
    for r in rutas:
        rows.append({"vehiculo_base": r, "capacidad": float(cap_map.get(r, DEFAULT_CAPACIDAD))})
...
def optimizar_cached(puntos, flota, escenario, usar_osrm_matrix=True, usar_osrm_geometry=True):
    ...
    def construir_vehiculos(flota_base):
        ...
        for _, row in fb.iterrows():
            ...
            vehs.append({"vehiculo_base": row["vehiculo_base"], "vuelta_slot": 1, "bloque": "AM", "capacidad": cap, "slot_start": BLOQUE_AM_START_MIN, "slot_end": BLOQUE_AM_END_MIN})
            vehs.append({"vehiculo_base": row["vehiculo_base"], "vuelta_slot": 2, "bloque": "PM", "capacidad": cap, "slot_start": BLOQUE_PM_START_MIN, "slot_end": BLOQUE_PM_END_MIN})
        ...
        for route_id, asign in enumerate(asignaciones, start=1):
            ...
            for seq, item in enumerate(stops, start=1):
                ...
                stop = {
                    ...
                    "m3_por_os_max": float(p.get("m3_por_os_max", 0)), "volumen_ajustado": bool(p.get("volumen_ajustado", False)),
...
def optimizar_ruta_preservando_flota(puntos, flota_baseline, usar_osrm_geometry=True):
    ...
    for _, veh_row in flota.iterrows():
        ...
        bloques = [
            ("AM", 1, BLOQUE_AM_START_MIN, BLOQUE_AM_END_MIN, pveh[pveh["tw_end"] <= HORA_CORTE_HUB_MIN].copy()),
            ("PM", 2, BLOQUE_PM_START_MIN, BLOQUE_PM_END_MIN, pveh[pveh["tw_end"] > HORA_CORTE_HUB_MIN].copy()),
        ]
...
def metricas_baseline(baseline, dicruta, cols):
    ...
    if col_km:
        ...
        ajuste_vol = ajustar_volumen_m3(d[col_km], q_os_base, umbral_m3_por_os=1.0, m3_estandar_por_os=0.33)
...
default_clases = ["EXCLUSIVA PU"] if "EXCLUSIVA PU" in clases_disponibles else clases_disponibles
...
# ============================================================
def ejecutar_escenarios_dia(fecha, clases_sel, tipos_sel):
    ...
    puntos = preparar_puntos(baseline, cols, max_capacidad=max_capacidad_disponible)
...
with tab_analisis:
    ...
    with c1: clases_sel = excel_filter("Clase ruta", clases_disponibles, "ana_clase", default_clases)
    with c2: tipos_sel = excel_filter("Tipo ruta", tipos_disponibles, "ana_tipo", tipos_disponibles)
    ...
    with c3: indicador_sel = excel_single_filter("Variable", [nombres[i] for i in indicadores], "ana_var", nombres["productividad"])
    inv = {v:k for k,v in nombres.items()}; var = inv[indicador_sel]

    fechas = sorted(peu[(peu["clase_ruta"].isin(clases_sel)) & (peu["tipo_ruta"].isin(tipos_sel))]["fecha_operacion"].dropna().unique())
    rows=[]
    prog = st.progress(0, text="Ejecutando escenarios diarios automáticamente...") if fechas else None
    for i, f in enumerate(fechas):
        res = ejecutar_escenarios_dia(f, clases_sel, tipos_sel)
...
with tab_macro:
    ...
    with c1: clases_dia = excel_filter("Clase ruta", clases_disponibles, "dia_clase", default_clases)
    with c2: tipos_dia = excel_filter("Tipo ruta", tipos_disponibles, "dia_tipo", tipos_disponibles)
    fechas_dia = sorted(peu[(peu["clase_ruta"].isin(clases_dia)) & (peu["tipo_ruta"].isin(tipos_dia))]["fecha_operacion"].dropna().unique())
    with c3: fecha_sel = excel_single_filter("Día", fechas_dia, "fecha_macro", fechas_dia[0] if fechas_dia else None)
    if not fecha_sel:
        st.warning("No hay fechas disponibles."); st.stop()
    with st.spinner("Calculando baseline y simulaciones automáticamente..."):
        sim = ejecutar_escenarios_dia(fecha_sel, clases_dia, tipos_dia)
    ...
    <div class="day-card">
      ...
      <div class="day-card-detail">
        Reservas: <b>{num(met_dia.get('reservas'))}</b> · OS: <b>{num(met_dia.get('total_os'))}</b> · Volumen: <b>{dec(met_dia.get('volumen'))} m³</b> · Rutas baseline: <b>{num(met_dia.get('cantidad_rutas'))}</b>
      </div>
    </div>
    ''', unsafe_allow_html=True)
    ...
    with sub1:
        ...
        if not stops_bl.empty:
            ...
            if cols["col_km"]:
                ...
                ajuste_mapa = ajustar_volumen_m3(stops_bl[cols["col_km"]], q_os_mapa, umbral_m3_por_os=1.0, m3_estandar_por_os=0.33)
...
with tab_exportar:
    ...
    c1, c2 = st.columns(2)
    with c1:
        clases_exp = excel_filter("Clase ruta descarga", clases_disponibles, "exp_clase", default_clases)
    with c2:
        tipos_exp = excel_filter("Tipo ruta descarga", tipos_disponibles, "exp_tipo", tipos_disponibles)

    fechas_exp = sorted(peu[(peu["clase_ruta"].isin(clases_exp)) & (peu["tipo_ruta"].isin(tipos_exp))]["fecha_operacion"].dropna().unique())
    ...
    if not fechas_sel_exp:
        ...
        if st.button("Preparar Excel de descarga", type="primary"):
            ...
            for i, f in enumerate(fechas_sel_exp):
                sim_tmp = ejecutar_escenarios_dia(f, clases_exp, tipos_exp)
